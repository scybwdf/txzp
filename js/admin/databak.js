...
function backup()
{
    ...
    if(confirm("确定要备份当前数据吗？"))
        swal({
            title:"请勿刷新本页，请稍候...",
            text:"<div id='dump_result' style='line-height:80px; text-align:center; color:#f30;'>正在备份数据库</div>",
            html:true,
...
function dump_sql(filebase_name,vol,table_key,last_row)
{
    ...
    query.filebak_name = filebase_name;
    query.ajax= 1;
    $.ajax({
        url: dump,
        data: query,
        ...
        success: function(obj){
            if(obj.status)
            {
                if(obj.done==0)
                {
                    ...
                    dump_sql(obj.filebak_name,obj.vol,obj.table_key,obj.last_row);
                }
                else
                {
...
{
    $("#info").remove();
    if(confirm("确定要恢复"+filename+"到当前数据库吗？"))
    swal({
        title:"请勿刷新本页，请稍候...",
        text:"<div id='restore_result' style='line-height:80px; text-align:center; color:#f30;'>正在恢复数据库备份"+filename+"_1.sql</div>",
        html:true,
        showConfirmButton:false
    });
...
{
    if(confirm("确定要删除"+filename+"备份数据吗？"))
        $.ajax({
            url: deletebak+'?file='+filename,
            data: "ajax=1",
            dataType: "json",
            success: function(obj){
                $("#info").html(obj.info);
