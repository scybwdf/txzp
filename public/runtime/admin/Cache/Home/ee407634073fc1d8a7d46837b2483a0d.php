...
<div class="ibox float-e-margins">
    <div class="ibox-title">
        <h5 class="text-primary text-center">编辑管理员</h5>
    ...
    <div class="ibox-content">
        <div class="row row-lg">
            <div class="col-xs-12">
                <form name="addadmin" class="form-horizontal" method="post" action="<?php echo U('edit');?>">
                    <div class="form-group col-sm-12">
                        <label class="control-label col-sm-3">用户名：</label>
                        <div class="col-sm-6">
                            <input type="text" class="form-control" name="username" required="" placeholder="" value="<?php echo ($v["username"]); ?>">
                        </div>
                    </div>
                    <div class="form-group col-sm-12">
                        <label class="control-label  col-sm-3">密码：</label>
                        <div class="col-sm-6">
                            <input type="password" class="form-control" name="password" placeholder="若密码为空则保持不变">
                        </div>
                    </div>
                    <div class="form-group col-sm-12">
                    	<label class="control-label  col-sm-3">所属管理组：</label>
                    	 <div class= "col-sm-6">
                              <select name="uid" class="form-control chosen-select" >
                                  <?php if(is_array($list)): foreach($list as $key=>$lv): ?><option value="<?php echo ($lv["id"]); ?>" <?php if($v['uid'] == $lv['id']): ?>selected="selected"<?php endif; ?> ><?php echo ($lv["title"]); ?></option ><?php endforeach; endif; ?>
                              </select>
                         </div>
                    </div>
