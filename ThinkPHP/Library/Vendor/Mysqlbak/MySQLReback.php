...
class MySQLReback {
    ...
    private function connect() {
        if ($this->conn=mysqli_connect($this->config['host'] . ':' . $this->config['port'], $this->config['userName'], $this->config['userPassword'])) {
    ...
    private function getTables($dbName) {
        @$db = mysqli_query($this->conn,"use $dbName");
        $rs=mysqli_query($this->conn,'show tables');
        $rows = mysqli_num_rows($rs);
        ...
        for ($i = 0; $i < $rows; $i++) {
            $tbName =$tbNames[$i]['Tables_in_'.$dbName];
    ...
    public function backup() {
        ...
        foreach ($this->dbName as $dbName) {
            ...
            $rs = mysqli_query($this->conn,"SHOW CREATE DATABASE {$qDbName}");
