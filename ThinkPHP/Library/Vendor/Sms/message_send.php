...
class message_send extends Controller {
	...
	const MAIL_CHANGE_VERIFY = 'TPL_MAIL_CHANGE_USER_VERIFY';
	...
	public function msg_reply($type,$tmlp='',$data){
		if($type=='email'||$type=='sms'){
			if(C("MAIL_ON")!=1&&$type=='email'){
				return false;
			}elseif(C("SMS_ON")!=1&&$type=='sms'){
				return false;
			}
			$this->msg_tmpl($tmlp);
			if($type=='email'){
	...
	public  function  get_user_info($dest,$type = 'email'){
		if($GLOBALS['user_info']['id']){
			...
			if($type=='email'){
				...
				$user_info = M('user')->where(array('mobile'=>$dest))->find();
			}
		}
		return $user_info;
	}
 	 	...
 	 	//$user_info = array();
  	 	if($type == 'email'){
 			...
 			if(!empty($dest)){
				...
				$msg_data['send_type'] = 0;
				$msg_data['code'] =$code;
				return $this->wf_send('sms','TPL_SMS_VERIFY_CODE',$msg_data);
  			}
  	 	}
 	 }
	/*
	...
	public function mail_demo($mail){
 	...
 	/*new短信验证码发送*/
 	public function wf_send($type,$tmlp='',$data){
		if($type=='sms'){
			Vendor('Sms.sms');
			$sms=new sms_sender();
			$tmpl=M('MsgTemp')->where(array('name'=>'短信验证码发送'))->find();
			$tmpl_content=$tmpl['content'];
			$verify['mobile'] = $data['dest'];
			$verify['code'] = $data['code'];
			$this->assign("verify",$verify);
			$msg=$this->fetch('',$tmpl_content);
			$result = $sms->sendSms($verify['mobile'],$msg);
			return $result;
		}
	 ...
	 */
	 ...
	 public function manage_msg($msg_type,$dest='',$param=array()){
	 	...
	 	switch($this->msg_type){
	 		...
	 		$aa=$this->msg_verify('sms',$dest,$param['code'],$param['title']);	
			return $aa;	
	 		break;
