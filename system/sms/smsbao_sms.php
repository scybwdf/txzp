...
require_once WEB_ROOT."/system/sms/smsbao/transport.php"; 
...
class smsbao_sms implements sms
{
  ...
  public function __construct($smsInfo = '')
	...
	public function check_fee()
	{
		//es_session::start();
		session('[start]');
		$last_visit = intval($_SESSION['last_visit_smsbao']);
    	...
    	else
    	{
			...
			session("smsbao_info",$str);
			session("last_visit_smsbao",time());
			return $str;
		}
		else
		{
			$qxt_info = $_SESSION['smsbao_info'];
			if($smsbao_info)
			return $smsbao_info;
			else
